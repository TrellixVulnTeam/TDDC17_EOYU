...
public class QLearningController extends Controller {
	...
	static final double LEARNING_RATE_CONSTANT = 10; /* 10 innan See alpha(), lower values are good for quick results in large and deterministic state spaces */
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			double previous_reward = StateAndReward.getRewardHover(previous_angle, previous_vx, previous_vy, y.getValue());
			action_counter = 0;
			...
			if (previous_state != null) {
				...
				double Qupdate = (1-alpha(Ntable.get(prev_stateaction)))*Qtable.get(prev_stateaction) + 
						alpha(Ntable.get(prev_stateaction)) * (previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state));
				Qtable.put(prev_stateaction, Qupdate);
